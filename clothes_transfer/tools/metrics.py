...
from clothes_transfer.tools.inception import InceptionV3
import lpips
...
def load_generated_images_(images_folder, shrink_rate=1, sharp_rate=1):
    ...
    for img_name in os.listdir(images_folder):
        ...
        if img.shape[1] % 3 == 0:
            ...
            input_images.append(addBounding(img[:, :w]))
            target_images.append(addBounding(img[:, w:2*w]))
            generated_images.append(addBounding(img[:, 2*w:3*w], shrink_rate, sharp_rate))
        else:
            w = int(img.shape[1] / 5) #h, w ,c

            input_images.append(addBounding(img[:, :w]))
            target_images.append(addBounding(img[:, 2*w:3*w]))
            generated_images.append(addBounding(img[:, 4*w:5*w], shrink_rate, sharp_rate))
...
if __name__ == "__main__":
    ...
    for shrink_rate in [1]: #range(100,50,-5):
        ...
        input_images, target_images, generated_images = load_generated_images(args.output_path, shrink_rate=shrink_rate, sharp_rate=sharp_rate, w=w)
