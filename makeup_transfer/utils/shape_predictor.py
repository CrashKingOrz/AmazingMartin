...
def get_landmark_from_direct(filepath,predictor):
    """get landmark with dlib
    :return: np.array shape=(68, 2)
    """
    detector = dlib.get_frontal_face_detector()
...
def align_face(img, predictor):
    ...
    lms = get_landmark(img, predictor)
    imgs = []
    for lm in lms:
...
def align_face_from_direct(filepath, predictor):
    """
    :param filepath: str
    :return: list of PIL Images
    """

    lms = get_landmark_from_direct(filepath, predictor)
    imgs = []
    for lm in lms:
        lm_chin = lm[0: 17]  # left-right
        lm_eyebrow_left = lm[17: 22]  # left-right
