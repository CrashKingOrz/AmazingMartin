...
class MakeupDataset(data.Dataset):
    def __init__(self, opts):
        ...
        # non_makeup
        name_non_makeup = os.listdir(os.path.join(self.dataroot, 'non_makeup'))
        self.non_makeup_path = [os.path.join(self.dataroot, 'non_makeup', x) for x in name_non_makeup]
    ...
    def __getitem__(self, index):
        if self.opt.phase == 'test_pair':
            ...
            non_makeup_parse = self.load_parse(self.non_makeup_path[non_makeup_index].replace('head', 'seg'), non_makeup_angle)

            makeup_img = self.load_img(self.makeup_path[makeup_index], makeup_angle)
            makeup_parse = self.load_parse(self.makeup_path[makeup_index].replace('head', 'seg'), makeup_angle)
